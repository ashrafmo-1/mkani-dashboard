  ...
  return (
    <div className="relative overflow-x-auto w-full px-10 my-20 pb-2 sm:rounded-lg">
      <div className="flex items-center justify-between mb-10">
        <h1 className="text-4xl font-bold text-gray-800 capitalize"> {t("roles.title")} </h1>
      ...
      <div className="filter mb-6 shadow p-4 rounded-lg">
        ...
        <input type="search" name="search" className="border rounded outline-none py-1 px-3 w-[400px]" id="search" placeholder="search" 
        ...
        // onChange={(value) => setFilter({ status: value })} placeholder={t("Select Status")}
        >
          ...
          <tbody>
            {Array.from({ length: 10 }).map((_, index) => (
                <tr className="bg-white border-b" key={index}>
                  <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{"Fake Name " + (index + 1)}</th>
                  <td className="px-6 py-4">{"123-456-789" + index}</td>
                  <td className="px-6 py-4">{"123 Fake St, Faketown, FK " + index}</td>
